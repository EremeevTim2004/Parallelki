...
#define MPI_BSEND_OVERHEAD MSMPI_BSEND_OVERHEAD_V1
    ...
    _Success_(return == MPI_SUCCESS && *flag != 0) int MPIAPI
        MPI_Test(
            _Inout_ _Post_equal_to_(MPI_REQUEST_NULL) MPI_Request *request,
    ...
    _Success_(return == MPI_SUCCESS && *outcount > 0) int MPIAPI
        MPI_Testsome(
            _In_range_(>=, 0) int incount,
            _mpi_updates_(incount) MPI_Request array_of_requests[],
            _mpi_out_range_(outcount, MPI_UNDEFINED, incount) int *outcount,
...
#define MPI_IN_PLACE ((void *)(MPI_Aint) - 1)
    ...
    typedef void(MPIAPI MPI_User_function)(
        _In_opt_ void *invec,
...
#define MPI_KEYVAL_INVALID 0x24000000
    ...
    typedef int(MPIAPI MPI_Comm_copy_attr_function)(
    ...
    typedef int(MPIAPI MPI_Comm_delete_attr_function)(
        _In_ MPI_Comm comm,
        _In_ int comm_keyval,
...
#define MPI_APPNUM 0x6440000d
    ...
    typedef int(MPIAPI MPI_Win_copy_attr_function)(
        _In_ MPI_Win oldwin,
        _In_ int win_keyval,
        _In_opt_ void *extra_state,
        _In_opt_ void *attribute_val_in,
    ...
    typedef int(MPIAPI MPI_Win_delete_attr_function)(
        _In_ MPI_Win win,
        _In_ int win_keyval,
        _In_opt_ void *attribute_val,
        _In_opt_ void *extra_state);
...
#define MPI_WIN_UNIFIED 2
    ...
    typedef int(MPIAPI MPI_Type_copy_attr_function)(
    ...
    typedef int(MPIAPI MPI_Type_delete_attr_function)(
...
#define MPI_MAX_PROCESSOR_NAME 128
    ...
    typedef void(MPIAPI MPI_Comm_errhandler_fn)(
        _In_ MPI_Comm *comm,
        _Inout_ int *errcode,
        ...);
    ...
    typedef void(MPIAPI MPI_Win_errhandler_fn)(
        _In_ MPI_Win *win,
        _Inout_ int *errcode,
        ...);
    ...
    typedef void(MPIAPI MPI_File_errhandler_fn)(
        _In_ MPI_File *file,
        _Inout_ int *errcode,
        ...);
...
#define MPI_MAX_PORT_NAME 256
    ...
    MPI_Fetch_and_op(
        ...
        _When_(target_rank != MPI_PROC_NULL, _In_)
            _When_(target_rank == MPI_PROC_NULL, _In_opt_) void *result_addr,
        _In_ MPI_Datatype datatype,
        _In_range_(>=, MPI_PROC_NULL) int target_rank,
        _In_range_(>=, 0) MPI_Aint target_disp,
        _In_ MPI_Op op,
...
#define MPI_LOCK_SHARED 235
    ...
    typedef int(MPIAPI MPI_Grequest_query_function)(
        _In_opt_ void *extra_state,
        _Out_ MPI_Status *status);

    typedef int(MPIAPI MPI_Grequest_free_function)(
        _In_opt_ void *extra_state);

    typedef int(MPIAPI MPI_Grequest_cancel_function)(
        _In_opt_ void *extra_state,
        _In_ int complete);
...
#define MPI_SEEK_END 604
    ...
    typedef int(MPIAPI MPI_Datarep_conversion_function)(
    ...
    typedef int(MPIAPI MPI_Datarep_extent_function)(
        _In_ MPI_Datatype datatype,
...
#define MSMPI_Get_version() (MSMPI_VER)
    ...
    typedef void(MPIAPI MSMPI_Request_callback)(
        _In_ MPI_Status *status);
